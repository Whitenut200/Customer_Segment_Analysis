# RFM·코호트·이탈 분석을 통한 상위 고객 구매 유지율 하락 문제 정의 및 가설 검증
...
- **기여도:**  
**100% (단독 수행)**  
기획 → Spark 전처리 → SQL 지표 산출 → 분석 → 시각화 → 문제 정의 → 가설 수립 → 실험 설계 → 결론 도출까지 **전 과정 수행**
...
### 5) 문제 정의 및 우선순위 선정
- 전 단계 분석 결과와 대시보드 인사이트를 기반으로 문제 정의
- 총 **4개의 문제점**을 도출하고, 선정 기준에 따라 우선순위 부여
- 1순위 문제에 대한 **최종 의사결정 지원**을 위해 가설 검정 진행

---

### 6) 가설 검정 및 실험 설계
- 원인 파악을 위한 다수 가설 수립
- 가설별 필요한 지표를 산출하고, 적절한 검정 방법을 설계
- 검정을 수행하고 결과를 해석하여 결론 도출
- 결론 기반으로 **실행 가능한 실험/개선 방향** 제안
...
| **검정 방법**| 카이제곱, 로지스틱 회귀, 상관분석, Wilcoxon signed-rank |
